...
export function extractFAQs(markdownContent) {
  const faqs = [];
  const qaPattern = /\*\*Q:\s*(.+?)\*\*\s*A:\s*(.+?)(?=\n\n|\n\*\*Q:|$)/gs;
  let match;

  while ((match = qaPattern.exec(markdownContent)) !== null) {
    faqs.push({
  ...
  const faqHeadingPattern = /##\s*FAQ|##\s*자주\s*묻는\s*질문/gi;
  const faqSectionIndex = markdownContent.search(faqHeadingPattern);

  if (faqSectionIndex !== -1 && faqs.length === 0) {
    const faqSection = markdownContent.slice(faqSectionIndex);
    const h3Pattern = /###\s*(.+?)[\r\n]+(.+?)(?=###|$)/gs;
    while ((match = h3Pattern.exec(faqSection)) !== null) {
      const question = match[1].trim();
      const answer = match[2].trim().split('\n')[0];

      if (question.length < 200 && answer.length > 10 && answer.length < 500) {
